...
export const validatePayment = expressAsyncHandler(async (req, res) => {
  ...
  if (unaccepted === true) {
    ...
    const isBought = await prisma.payment.findUnique({
      where: {
        order_id: label,
      },
      select: {
    ...
    if (isBought.status === false) {
      try {
        ...
        await bot_tg.sendMessage(obj.tgId, `Благодарим за покупку`);
        await bot_tg.sendDocument(obj.tgId, "./public/СОЗДАНИЕ БИРОК.zip");
      } catch (error) {
        console.log(error);
        const obj = await prisma.payment.findUnique({
          where: {
      ...
      console.log(`${label} - повторная проверка.`);
    }
  }
});
